<div id="content" class="p-4 p-md-5 pt-5">
<div id="label-page"><h3>Tampil Data Transaksi</h3></div>
<div class="form-inline">
    <button type="button" class="btn btn-primary">
        <a href="index.php?p=transaksi-input" class="tombol" style="color:white">Tambah Buku</a></button>
            <div style="right:0px">
                <form method=post style="margin-left:730px">
                    <input type="text" name="pencarian">
                    <input type="submit" name="search" value="search" class="tombol">
                </form>
            </div>
        </div>

    <!--table-->
    <table id="example" class="table table-hover">
        <thead>
            <tr>
            <th scope="col">No</th>
            <th scope="col">ID Transaksi</th>
            <th scope="col">ID Anggota</th>
            <th scope="col">ID Buku</th>
            <th scope="col">Tanggal Pinjam</th>
            <th scope="col">Tanggal Kembali</th>
            <th scope="col">Status</th>
            <th scope="col">Aksi</th>
            </tr>
        </thead>
        <tbody>
        <?php
            $batas = 10;
            extract ($_GET);
            if(empty($hal)){
                $posisi = 0;
                $hal = 1;
                $nomor = 1;
            }else{
                $posisi = ($hal-1)*$batas;
                $nomor = $posisi+1;
            }
            if($_SERVER['REQUEST_METHOD']=="POST"){
                $pencarian = trim(mysqli_real_escape_string($db, $_POST['pencarian']));
                if($pencarian != ""){
                    $sql = "SELECT * FROM tbtransaksi WHERE idpeminjaman LIKE '%$pencarian%'
                    OR idanggota LIKE '%$pencarian%'
                    OR idbuku LIKE '%$pencarian%'
                    OR tanggalpinjam LIKE '%$pencarian%'
                    OR tanggalkembali LIKE '%$pencarian%'";
                    $query = $sql;
                    $queryJml = $sql;
                } else{
                    $query= "SELECT * FROM tbtransaksi LIMIT $posisi, $batas";
                    $queryJml = "SELECT * FROM tbtransaksi";
                    $no = $posisi * 1;
                }
            }
            else{
                $query = "SELECT *FROM tbtransaksi LIMIT $posisi, $batas";
                $queryJml = "SELECT * FROM tbtransaksi";
                $no = $posisi*1;
            }
            $q_tampil_peminjaman = mysqli_query($db, $query);
            if(mysqli_num_rows($q_tampil_peminjaman)>0){
                while($r_tampil_peminjaman=mysqli_fetch_array($q_tampil_peminjaman)){
        ?>
                <tr>
                    <td><?php echo $nomor;?></td>
                    <td><?php echo $r_tampil_peminjaman['idpeminjaman'];?></td>
                    <td><?php echo $r_tampil_peminjaman['idanggota'];?></td>
                    <td><?php echo $r_tampil_peminjaman['idbuku'];?></td>
                    <td><?php echo $r_tampil_peminjaman['tanggalpinjam'];?></td>
                    <td><?php echo $r_tampil_peminjaman['tanggalkembali'];?></td>
                    <td><?php echo $r_tampil_peminjaman['status'];?></td>
                    <td>
                        <button type="button" class="btn btn-success btn-sm"><a href="index.php?p=pengembalian-input&id=<?php echo $r_tampil_peminjaman['idpeminjaman'];?>" class="tombol" style="color:white">Kembalikan</a></div>
                        <!-- <button type="button" class="btn btn-warning btn-sm"><a href="index.php?p=transaksi-edit&id=<?php echo $r_tampil_peminjaman['idpeminjaman'];?>" class="tombol" style="color:white">Edit</a></div> -->
                        <button type="button" class="btn btn-danger btn-sm"><a href="proses/peminjaman-hapus.php?id=<?php echo $r_tampil_peminjaman['idpeminjaman'];?>" onclick="return confirm('Apakah anda yakin akan menghapus data ini?')" class="tombol" style="color:white">Hapus</a></button>
                    </td>
                </tr>
                <?php
                $nomor++;
                }
            }
            else{
                echo "<tr><td colspan=6>Data tidak ditemukan</td></tr>";
            }
            ?>
            </tbody>
        </table>
        <?php
        if(isset($_POST['pencarian'])){
            if($_POST['pencarian']!=''){
                echo "<div style=\"float:left;\">";
                $jml = mysqli_num_rows(mysqli_query($db, $queryJml));
                echo "Data hasil pencarian: <b>$jml</b>";
                echo "</div>";
            }
        } else{
            ?>
            <div style="float: left;">
            <?php
            $jml= mysqli_num_rows(mysqli_query($db, $queryJml));
            echo "Jumlah data : <b>$jml</b>";
            ?>
            </div>
            <div class="pagination">
                <?php
                $jml_hal = ceil($jml / $batas);
                for($i=1; $i<=$jml_hal; $i++){
                    if($i !=$hal){
                        echo "<button><a href=\"?p=transaksi&hal=$i\">$i</a></button>";
                    }
                    else{
                        echo "<button><a class=\"active\">$i</a></button>";
                    }
                }
                ?>
            </div>
        <?php
        }
        ?>
</div>
    